...
<ul>
  <li><a href='<%=request.getContextPath()%>/front-end/bid/listAllBid.jsp'>List</a> all bids.  <br><br></li>
  ...
  <li>
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/bid/bid.do" >
       <b>選擇得標編號:</b>
       <select size="1" name="bid_no">
         <c:forEach var="bidVO" items="${bidSvc.all}" > 
          <option value="${bidVO.bid_no}">${bidVO.bid_no}
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_Display">
       <input type="submit" value="送出">
...
<ul>
  <li><a href='<%=request.getContextPath()%>/front-end/bid/addBid.jsp'>Add</a> a new bid.</li>
</ul>
