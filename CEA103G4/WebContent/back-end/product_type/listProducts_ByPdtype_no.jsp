...
<table>
	...
	<c:forEach var="productVO" items="${listProducts_ByPdtype_no}">
	<tr ${(empVO.empno==param.empno) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色-->
			...
			<td><c:forEach var="product_typeVO" items="${product_typeSvc.all}">
                    <c:if test="${productVO.pdtype_no==product_typeVO.pdtype_no}">
	                    ${product_typeVO.pdtype_no}【<font color=orange>${product_typeVO.pdtype_name}</font>】
                    </c:if>
                </c:forEach>
			</td>
